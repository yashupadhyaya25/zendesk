...
s3_folder_path = 'ticket_forms/' ### Folder path till /
s3_file_name = s3_folder_path+'ticket_forms_data.json'
url = "https://"+comapny_zendesk_org_name+".zendesk.com/api/v2/ticket_forms"
REQUEST_HEADER =  {
...
def main():
    ...
    res = ticket_form_fetch()
    return res

def ticket_form_fetch():
    try :
        api_data = []
        loop_flag = True
        next_page_url = ''
        while loop_flag :
            ...
            json_data = response.json().get('ticket_forms')
            next_page_url = response.json().get('next_page')
...
if __name__ == '__main__' :
    print('API Ticket Forms Started On : '+str(dt.now()))
    res = main()
    print(res)
    print('API Ticket Forms Completed On : '+str(dt.now()))