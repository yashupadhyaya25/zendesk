...
s3_folder_path = 'ticket_fields/' ### Folder path till /
s3_file_name = s3_folder_path+'ticket_fields_data.json'
url = "https://"+comapny_zendesk_org_name+".zendesk.com/api/v2/ticket_fields?page[size]=1"
next_url_page_file = s3_folder_path+"ticket_fields_cursor.txt"
REQUEST_HEADER =  {
...
def fetch_field_mapping():
    try :
        ...
        while loop_flag :
            ...
            json_data = response.json().get('ticket_fields')
...
if __name__ == '__main__' :
    print('API Ticket Fields Started On : '+str(dt.now()))
    res = main()
    print(res)
    print('API Ticket Fields Completed On : '+str(dt.now()))