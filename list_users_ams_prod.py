...
s3_folder_path = 'user/' ### Folder path till /
s3_file_name = s3_folder_path+'user_data.json'
url = "https://"+comapny_zendesk_org_name+".zendesk.com/api/v2/incremental/users/cursor?start_time=0"
next_url_page_file = s3_folder_path+"user_cursor.txt"
REQUEST_HEADER =  {
...
def user_fetch():
    try :
        ...
        while loop_flag :
            ...
            json_data = response.json().get('users')
...
if __name__ == '__main__' :
    print('API User Started On : '+str(dt.now()))
    res = main()
    print(res)
    print('API User Completed On : '+str(dt.now()))