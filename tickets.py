...
s3_folder_path = 'tickets/' ### Folder path till /
s3_file_name = s3_folder_path+'tickets_data.json'
url = "https://"+comapny_zendesk_org_name+".zendesk.com/api/v2/incremental/tickets/cursor.json?start_time=0"
next_url_page_file = s3_folder_path+"tickets_cursor.txt"
REQUEST_HEADER =  {
...
def ticket_fetch():
    try :
        ...
        # try :
        #     status = s3_client.get_object(Key = next_url_page_file,
        # Bucket = s3_bucket_name,
        ...
        while loop_flag :
            ...
            json_data = response.json().get('tickets')
            has_more = response.json().get('end_of_stream')  
            next_page_url = response.json().get('after_url')
...
if __name__ == '__main__' :
    print('API Ticket Started On : '+str(dt.now()))
    res = main()
    print(res)
    print('API Ticket Completed On : '+str(dt.now()))